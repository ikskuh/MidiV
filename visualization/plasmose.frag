...
// Contains a 1D texture per channel with all note values in `.r` and all CC values in `.g`
...
void main()
{
	...
	float v = 0
		+ 0.6 * sin(1.1 * uTime + 23.0 * xy.x)
		+ 0.5 * sin(0.2 * uTime + 19.4 * xy.y)
		+ 0.4 * sin(1.3 * uTime + 17.0 * xy.x)
		+ 0.3 * sin(1.4 * uTime + 13.2 * xy.y)
		+ 0.2 * sin(0.5 * uTime + 11.0 * xy.x)
		+ 0.1 * sin(1.6 * uTime +  7.0 * xy.y)
		;
